...
class SettingsRepository(private val context: Context) {
    ...
    // Ghost Piece
    val ghostPieceEnabled: Flow<Boolean> = context.dataStore.data
        .catch { if (it is IOException) emit(emptyPreferences()) else throw it }
        .map { it[GHOST_PIECE_ENABLED] ?: true }
    
    suspend fun setGhostPieceEnabled(enabled: Boolean) {
        context.dataStore.edit { it[GHOST_PIECE_ENABLED] = enabled }
    }
}
